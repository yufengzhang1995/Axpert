...
from impressions_dataset import ImpressionsDataset,BootstrappedImpressionsDataset
...
def load_data(train_csv_path, 
     ...
     if bootstrap:
          print('Bootstrap')
          train_dset = BootstrappedImpressionsDataset(train_csv_path, train_list_path, seed = seed)
     else:
          print('Regular')
     ...
     dev_loader = torch.utils.data.DataLoader(dev_dset, 
                                   batch_size=batch_size, 
                                   shuffle=shuffle,
                                   num_workers=num_workers, 
                                   collate_fn=collate_fn)
...
def train(save_path, 
     ...
     loss_func = nn.CrossEntropyLoss(weight=torch.tensor(WEIGHTED_LOSS, dtype = torch.float32, device=device))
               ...
               if (i+1) % valid_niter == 0:
                    ...
                    for j in range(len(CONDITIONS)):
                         print('%s f1_neg: %.3f, f1_pos: %.3f,  f1_uncertain: %.3f,'
                                   ...
                                   metrics['f1'][j][1],
                                   metrics['f1'][j][2],
                                   metrics['precision'][j][0],
                                   metrics['precision'][j][1],
                                   metrics['precision'][j][2],
                                   metrics['recall'][j][0],
                                   metrics['recall'][j][1],
                                   metrics['recall'][j][2],
                                   metrics['accuracy'][j]
                    ...
                    if metric_avg > best_metric: #new best network
                         ...
                         with open(metrics_path, 'w', newline='') as csvfile:
                              ...
                              csv_writer.writerow(['Condition', 'F1_neg', 'F1_pos', 'F1_uncertain',
                                             ...
                                             'Recall_neg', 'Recall_pos', 'Recall_uncertain',
                                             'Accuracy','average_Kappa'])
...
if __name__ == '__main__':
     ...
     parser.add_argument('--clinical', action='store_true', help='clinicalbert')
     ...
     # --bootstrap to default to False and only be True when explicitly specified:
     parser.add_argument('--bootstrap', action='store_true', help='Toggle bootstrap mode on')
     parser.add_argument('--seed', type=int, default = 1234, help='bootstrap seed ')
     ...
     if clinical:
          out_path = os.path.join(out_path,'clinical')
          if not os.path.isdir(out_path):
               os.mkdir(out_path)
          out_path = os.path.join(out_path,f'rand_{seed}')
          if not os.path.isdir(out_path):
               os.mkdir(out_path)
          
     else:
          out_path = os.path.join(out_path,'uncased')
          if not os.path.isdir(out_path):
               os.mkdir(out_path)
          out_path = os.path.join(out_path,f'rand_{seed}')
          if not os.path.isdir(out_path):
     ...
     if not os.listdir(out_path): # empty
          if checkpoint_path:
               model, optimizer, device = model_from_ckpt(bert_labeler(clinical), checkpoint_path)
          else:
               model, optimizer, device = None, None, None
          f1_weights = utils.get_weighted_f1_weights(dev_csv_path)
          dataloaders = load_data(train_csv_path, train_imp_path, dev_csv_path, dev_imp_path, bootstrap = bootstrap, seed = seed)
          ...
          print(f"{out_path} is not empty. Exiting.")
          exit()