...
def compute_metrics(y_true, y_pred):
    """Compute the mention F1 score as in CheXpert paper
    @param y_true (list): List of 1 tensors each of shape (dev_set_size)
    @param y_pred (list): Same as y_true but for model predictions

    @returns res (list): List of 1 scalars
    """
    # for j in range(len(y_true)):
    #     y_true[j][y_true[j] == 2] = 1
    #     y_pred[j][y_pred[j] == 2] = 1
    ...
    accuracy = []
    for j in range(len(y_true)): 
        f1.append(f1_score(y_true[j], y_pred[j], zero_division=0,average=None))
        recall.append(recall_score(y_true[j], y_pred[j], zero_division=0,average=None))
...
def evaluate(model, dev_loader, device, f1_weights, return_pred=False):
    ...
    if was_training:
        model.train()
    f1,recall,precision,accuracy = compute_metrics(copy.deepcopy(y_true), copy.deepcopy(y_pred))
