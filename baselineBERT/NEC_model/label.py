...
def label(checkpoint_path, csv_path, test = False, list_path = None):
    ...
    print(f'Inference time using {device}: mean and std are {np.mean(time_records)} and {np.std(time_records)}.')    
    if was_training:
        model.train()

    y_pred = np.array([t.tolist() for t in y_pred]).squeeze()
    y_gt = np.array([t.tolist() for t in y_gt]).squeeze()
    ...
    for cls in ['neg','pos','uncertain']:
        f1,recall,precision,accuracy = compute_metrics(copy.deepcopy(y_gt), copy.deepcopy(y_pred),cls = cls)
        f1s.append(f1)
